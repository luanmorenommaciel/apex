...
| G6 oraculo agendado sintetico vs real drift | Parcial | `tools/g6_oracle_drift_smoke.py` gera sintetico oficial, roda `oracle/compare.py` contra `real_log.ndjson` e salva resumo JSON; evidencia em `evidence/g6-oracle-drift-smoke.log` e `evidence/g6-oracle-drift-summary.json`. | Smoke local verde; falta agendamento/infra CI para virar drift continuo. |
