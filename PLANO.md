...
| --- | --- | --- |
| L1 Pipeline: Spark Envy Docker -> SparkListener -> ClickHouse -> Crew.ai -> MCP | Cumpre localmente | Existe ClickHouse/store, MCP stdio/subprocess, compose autonomo paralelo, listener JVM promovido para caminho oficial dos jobs, G3/G5 completos reexecutados em Spark 4.1.2 na stack autonoma, MCP aprovado/validado em Claude Code GUI e `crew_judge_diagnose` como tool read-only com provider Crew.ai opcional. Em 19/07, Crew.ai com LLM externo real foi observado com `provider=crew_ai` e `status=judged`. |
...
| L5 Diagnostico agentico Crew.ai explica o problema | Cumpre localmente | Diagnostico T1 continua deterministico em `apex/commander/diagnostic_mvp.py`; `apex/commander/crew_judge.py` adiciona `crew_judge_diagnose` read-only após T1 + `EvidenceValidator`; `apex/commander/judge_contract.py` valida decisão/citações; `apex/commander/judge_providers.py` tem provider deterministico, fallback `noop` e provider Crew.ai opcional por `APEX_CREW_JUDGE_ENABLED=1`. Evidencia final: `evidence/crew-judge-external-llm-success-final-2026-07-19.json`. |
...
| L9 Minimo viavel de cada componente antes de expandir qualquer um | Cumpre localmente | Ha MVPs locais para ingest, store, detectores, validator, MCP GUI, apply guardado, rerun, Docker/Compose autonomo, SparkListener JVM real/fail-safe e Crew/Judge plugável com execução LLM externa observada. |
...
| G4 T1 < 1s sem LLM; LLM so confidence < 0.6 | Cumpre localmente | `evidence/g4-t1.log` mede o caminho T1 deterministico contra `app-20260712053414-0001`: 226.991 ms, com `EvidenceValidator` aceitando o finding e grep sem referencias a LLM/API no caminho medido; `crew_judge_diagnose` implementa escalonamento read-only sem entrar no caminho quente; LLM externo foi observado separadamente em `evidence/crew-judge-external-llm-success-final-2026-07-19.json`. | T1 segue sem LLM obrigatório; Judge externo existe apenas como camada opcional pós-validação. |
...
| L9 | Fechado localmente | Nao colocar LLM no caminho quente; preservar regressao G0-G6, stack autonoma, schema, listener, MCP GUI e Judge contract verdes. |
...
7. **Etapa 7 — Crew.ai**
   ...
   - Execucao com `APEX_CREW_JUDGE_ENABLED=1` e `--allow-external-llm` fechou com `provider=crew_ai` e `status=judged`.
   - Proximo passo: ampliar matriz de prompts/casos e manter segredo fora do repo.
