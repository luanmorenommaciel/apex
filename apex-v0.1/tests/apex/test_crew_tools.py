...
from apex_diagnostics.detectors.plans import PLAN_TEXT_SQL, PLANS_SQL
...
SKEWED = [{"stage_id": 4, "max_ms": 45000, "p50_ms": 3000, "n_tasks": 24}]


def make_client() -> FakeCHClient:
    return FakeCHClient(
        {SKEW_SQL: SKEWED, SHUFFLE_SQL: [], PLANS_SQL: [], PLAN_TEXT_SQL: [], GC_SQL: [], OOM_SQL: []}
    )


def test_detector_tools_run_without_llm():
    tools = build_detector_tools(make_client(), THRESHOLDS)
    assert [tool.name for tool in tools] == ["detect_skew", "detect_shuffle", "detect_plans"]
