---
id: T-20260820-l6-tool-surface
title: "Re-pin the tool surface for recall_similar_runs"
status: done
format_version: 3
profile: standard
effort: XS
...
# Re-pin the tool surface for recall_similar_runs

> **Why:** An unnoticed tool on a server a model can call is a security event.

## Goal

Move the ordered assertion to include recall_similar_runs, keeping it an exact equality.
...
- **B-1** — GIVEN the built server WHEN list_tools() is called THEN the assertion is an exact ordered list including recall_similar_runs
