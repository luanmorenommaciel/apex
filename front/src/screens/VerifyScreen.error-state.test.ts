...
describe("VerifyScreen query-error state", () => {
  it("handles a rejected verification query before genuine absence", () => {
    const errorBranch = source.indexOf("if (fixQ.error)");
    ...
    expect(source).toContain('{fixQ.error.name || "Error"}');
  ...
  it("preserves the real zero-row state", () => {
    ...
    expect(source).toContain("This is a gap in the pipeline, not a failure of the query");
  });
});
