...
- **Re-analysis converges.** `apex.findings` is a plain MergeTree with no dedup, so `analyze()` inserts only findings the job does not already have. The signature is `(stage_id, type, detected_by, evidence)` — which is why `evidence` carries only what re-analysis of the same job reproduces byte-for-byte. The measured floor (pct, sample count) and ratio spread move every time a sibling run lands, so they live in `details` (never persisted), not in `evidence`.
- **Identity must not be derived from something that quietly moves.** Three instances of one class in this lane: `argMax(plan_fingerprint, ts) AS plan_fingerprint` shadowed the filter column, ClickHouse resolved the WHERE to the aggregate, and the shape read failed into a silently absent floor; `FixedString(64)` came back as `bytes` and every shape key failed to match just as silently; and a measured floor rendered into `evidence` shifted the dedup signature whenever history grew, so re-analysis accumulated rows instead of converging. Same shape each time: a key derived from a value that does not hold still. Keys and signatures are now built only from values that are stable by construction, and silent degradation is surfaced (`store_warnings`) rather than absorbed.
...
- **The one Tier-1 path that could escalate is closing.** It was a critical GC ratio computed against the `task_count × p50` *proxy* denominator, which scores `0.45` — below the `0.60` gate. Now that `executor_run_time_ms` is a typed column and a listener supplies it, that same finding is `measured` and scores `0.85` (critical) or `0.62`, both at or above the gate. So for runs emitting the column there is **no** escalating Tier-1 path left, and the crew lane goes dormant rather than merely unused. Rows predating the column still take the proxy — the read falls back to `attributes` and only reaches the proxy when neither exists — so the old path persists for historical jobs only. Whether the LLM lane should be re-pointed at something else is a product call, not a consequence this change should decide silently. Every other rule is confident when severe, or not severe when unsure. That is the gate being tight, not broken. Skew never escalates any more either: what used to make it ambiguous (the 5–10× band) is now settled by the closed form, and where it *is* unsettled the missing input is a cluster width, which no model can supply.
