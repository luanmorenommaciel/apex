...
| CODEX-010 | F2/G2 | bug | cenário oficial `skew_on_join_30x.yaml` disparou `shuffle_skew_candidate`, mas com severity `warning` abaixo do esperado `high` | `evidence/g2-cenarios.log`; `evidence/generated/official-scenarios/skew_on_join_30x.ndjson` | aberta |
| CODEX-011 | F2/G2 | bug | cenário oficial `gc_pressure_25pct.yaml` disparou `gc_pressure_candidate`, mas com severity `warning` abaixo do esperado `critical` | `evidence/g2-cenarios.log`; `evidence/generated/official-scenarios/gc_pressure_25pct.ndjson` | aberta |
| CODEX-012 | F2/G2 | bug | cenário oficial `shuffle_spill_disk.yaml` disparou `shuffle_spill_candidate`, mas com severity `warning` abaixo do esperado `critical` | `evidence/g2-cenarios.log`; `evidence/generated/official-scenarios/shuffle_spill_disk.ndjson` | aberta |
| CODEX-013 | F2/G2 | bug | cenário oficial `oom_on_aggregation.yaml` não disparou `oom_candidate` porque a evidência atual preserva apenas `ExceptionFailure`, sem a classe/descrição OutOfMemoryError | `evidence/g2-cenarios.log`; `evidence/generated/official-scenarios/oom_on_aggregation.ndjson` | aberta |
| CODEX-014 | F2/G2 | bug | cenário oficial `cartesian_product.yaml` não disparou detector de plano/cartesian product | `evidence/g2-cenarios.log`; `evidence/generated/official-scenarios/cartesian_product.ndjson` | aberta |
