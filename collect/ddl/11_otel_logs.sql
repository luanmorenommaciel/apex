-- Apex collect · exporter-owned OTLP logs schema (clickhouseexporter v0.156.0).
-- PINNED from the same empirical probe as otel_traces. Required because config.yaml wires
-- a logs pipeline (create_schema:false → table must pre-exist). jar currently emits spans
-- only, so this table is idle today, but it keeps the exporter's INSERT target valid and
-- the contract surface complete. 90-day TTL added (we own the schema).

CREATE TABLE IF NOT EXISTS apex.otel_logs
(
