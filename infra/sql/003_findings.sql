...
-- Pre-existing volumes: CREATE ... IF NOT EXISTS above is a no-op on a table created
-- before app_id/confidence_score existed (initdb only runs on FIRST boot — the silent-miss
-- bug apply-ddl exists to fix). These ALTERs close the gap idempotently on the live table.
ALTER TABLE apex.findings ADD COLUMN IF NOT EXISTS app_id           String  DEFAULT '' AFTER job_id;
ALTER TABLE apex.findings ADD COLUMN IF NOT EXISTS confidence_score Float32 DEFAULT 0  AFTER confidence;
