-- Apex infra · apex.fix_verifications — RATIFIED contract v0.3 schema (CONTRACT.md
-- changelog v0.3; ratified in v0.4, commit 5c19173). MIRROR of
-- verify/ddl/fix_verifications.ddl.sql (the verify lane owns this DDL; infra owns
-- applying it to the canonical store — the lane correctly never creates it itself).
-- Applied verbatim (only `IF NOT EXISTS` added; CREATE DATABASE dropped —
-- 001_database.sql owns it). Never rename/repurpose a column.
--
...
CREATE TABLE IF NOT EXISTS apex.fix_verifications (
  -- ── identity ────────────────────────────────────────────────────────────
