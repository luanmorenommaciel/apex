...
| **G3** Dado real | Sintético ≈ real no plat-v0 | oráculo dentro da tolerância + run multi-core 8 tasks | ✅ **verde 09/07** — run real `app-20260710021939-0000` (worker 8 cores): 8 tasks no stage do join, hot 164.956 records, **ratio real 29.4x vs sintético 27.9x** — oráculo na tolerância + watcher verde no log real (`scripts/g3_multicore_gate.py`) |
| **G4** Latência | Diagnóstico contínuo sem LLM obrigatório | T1 determinístico < 1s; LLM só quando confidence < threshold | ✅ **verde 09/07, validado com dado real 10/07** — pipeline completo no plat-v0: ingest → ClickHouse → T1 detectou skew 29.2x por `shuffle_records` em **333ms, zero tokens** (`t1-deterministic`), finding persistido. Bateu com o watcher Mundo A (29.4x). Lição incorporada: skew de registros ≠ skew de duração em dataset pequeno (commit 8fa225a) |
| **G5** Loop no IDE | MCP end-to-end no Cursor/Claude Code | `get_findings` → `apply_fix` com backup + diff revisável | ✅ **CICLO COMPLETO validado 10/07 com dado real**: skew 29.2x detectado (T1, 333ms, zero tokens) → `apply_fix` gerou AQE+salting+broadcast → revisão do diff pegou 2 bugs do código gerado (import fora de ordem; coluna duplicada) → job corrigido re-submetido ao plat-v0: **rodou limpo, shuffle eliminado (1.16MB→0), T1 = 0 findings**. Detectar→corrigir→provar, fim a fim. Falta só o registro no IDE (cosmético p/ demo ao vivo) |
...
**Sequência de implementação:** ~~G1~~ ✅ → ~~G2~~ ✅ → ~~G4~~ ✅ → ~~G3~~ ✅ → G5(executar o roteiro) → G6(secrets). Contrato de schema do merge: `docs/specs/telemetry-schema-contract-v1.md`.
