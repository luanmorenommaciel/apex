...
| | |
|---|---|
| **Shipped** | `apex-mcp` — stdio MCP server, seven tools (`analyze_run`, `explain_stage`, `compare_runs`, `list_runs`, `search_kb`, `verify_fix`, `suggest_fix`) and one resource (`apex://runs`) |
