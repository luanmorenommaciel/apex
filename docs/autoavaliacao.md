...
- O runner de regressão G3/G5 Spark 4.1.2 já foi criado em
  `scripts/f7_autonomous_stack_loop.py` e o workflow
  `Apex Autonomous Stack Loop` valida contrato/dry-run. Falta acionar o job
  real `real-stack` em runner self-hosted preparado com Docker/Compose e imagem
  base Spark 4.1.2.
...
- A parte mais fraca agora é produto/agência: Crew.ai real e UI de produto ainda
  ...
  G3/G5 já têm evidência; a execução remota real do novo loop ainda precisa de
  runner preparado.
