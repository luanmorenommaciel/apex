...
| --- | ---: | --- |
| C1 Arquitetura V1 (L1-L9) | 5/5 | `PLANO.md` classifica L1/L2/L3/L4/L5/L6/L7/L8/L9 como cumpridas localmente: há compose autônomo Spark 4.1.2, listener JVM promovido para caminho oficial dos jobs, G3/G5 reexecutados em Spark 4.1.2, MCP validado em Claude Code GUI real, `apply_fix` local e `crew_judge_diagnose` read-only com provider Crew.ai real observado em `evidence/crew-judge-external-llm-success-final-2026-07-19.json`. |
...
remoto verde, F7 real-stack remoto verde e Crew.ai com LLM externo real observado.
...
- Crew/Judge plugável agora existe como `crew_judge_diagnose`, com contrato
  ...
  opcional. O smoke final com `APEX_CREW_JUDGE_ENABLED=1` fechou com
  `provider=crew_ai`, `status=judged`, citações de evidência existentes e
  decisão conservadora `manual_review`, sem apply direto.
